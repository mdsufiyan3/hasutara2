...
document.addEventListener('DOMContentLoaded', function() {
    ...
    for (let i = 0; i < 3; i++) {
        ...
        categoryDescription.appendChild(shootingStar);
    }
    ...
    for (let i = 0; i < 50; i++) {
        ...
        categoryDescription.appendChild(star);
    }
...
// Update the cart count function to show total items
        ...
        function togglePlay() {
            if (video.paused) {
                ...
                document.querySelectorAll('.reel-video').forEach(v => {
                    if (v !== video) {
                        v.pause();
                        const otherPlayBtn = v.closest('.reel-card').querySelector('.play-btn');
...
// Add this after the menu overlay event listeners
function initializeMenuScroll() {
    ...
    function updateScrollButtonsVisibility() {
        ...
        const isScrollable = menuCategories.scrollHeight > menuCategories.clientHeight;
        const isAtTop = menuCategories.scrollTop <= 0;
        const isAtBottom = menuCategories.scrollTop + menuCategories.clientHeight >= menuCategories.scrollHeight;

        if (scrollUpBtn) {
            scrollUpBtn.classList.toggle('visible', isScrollable && !isAtTop);
        }
        if (scrollDownBtn) {
            scrollDownBtn.classList.toggle('visible', isScrollable && !isAtBottom);
        }
    }
    ...
    if (menuCategories && scrollUpBtn && scrollDownBtn) {
        ...
        scrollUpBtn.addEventListener('click', () => {
            menuCategories.scrollTo({
                top: menuCategories.scrollTop - scrollAmount,
                behavior: 'smooth'
            });
        });
